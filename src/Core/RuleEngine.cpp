...
std::vector<CaptureMove> RuleEngine::check_capture_all(const std::pair<int, int>& coordinate, const  std::vector<std::vector<int>>& int_grid)
{
...
std::vector<CaptureMove> RuleEngine::check_capture(const std::pair<int, int>& coordinate, const std::vector<std::vector<int>>& int_grid)
{
	std::vector<CaptureMove> coordinate_move;
	CaptureMove coordinate_elem;
	int size = int_grid.size();
...
bool RuleEngine::is_pat(const std::vector<std::vector<int>>& int_grid)
{
	...
	for (int i = 0; i < 8 && !found; i++)
	{
		...
		{
			if (check_capture_all({ i,j },int_grid).size() != 0) {
				found = true;
				break;
			}
